...
export function Modal({
  ...
  if (!open) return null
  return (
    <div className="fixed inset-0 flex items-center justify-center p-3 sm:p-4" style={{ background: 'rgba(0,0,0,.5)', zIndex }}>
      <div
        className="max-h-[88vh] w-full overflow-y-auto rounded-2xl border p-4 thin-scroll sm:p-6"
        style={{ background: 'var(--surface)', borderColor: 'var(--border)', maxWidth: width, boxShadow: 'var(--shadow)' }}
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-semibold font-display sm:text-lg">{title}</h2>
